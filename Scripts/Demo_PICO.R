...
if(parFlag==TRUE){
  ...
  arrayDir <- paste0(PICO_filePath, 'resamplingData/SMBO3/PICO_SMBO3_TEST_5r_resampArr.Rdata')
  
  # Run resampling (in parallel)
  PICO_demoArray_Par <- 
    geo.gen.Resample.Par(genObj=PICO_genind,  genDistFlag=TRUE, geoFlag=TRUE, coordPts=PICO_coordinates, 
                         geoBuff=geo_buffSize, SDMrast=PICO_sdm, boundary=world_poly_clip_W, 
  ...
  # Specify file path, for saving resampling array
  arrayDir <- paste0(PICO_filePath, 'resamplingData/SMBO2_G2E/PICO_TEST_resampArr.Rdata')
  # Run resampling not in parallel (for function testing purposes)
  PICO_demoArray_IND <-
    geo.gen.Resample(genObj = PICO_genind, genDistFlag=TRUE, SDMrast = PICO_sdm, geoFlag = TRUE, coordPts = PICO_coordinates,
                     geoBuff = 2000, boundary = world_poly_clip, ecoFlag = TRUE, ecoBuff=2000,
