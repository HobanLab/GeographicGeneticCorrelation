...
ARTH_SMBO2_array <- readRDS(arrayDir)
...
colnames(ARTH_NRMSE_Mat) <- c('Geo_Buff','Eco_Buff')
...
for(i in 3:ncol(ARTH_SMBO2_DF)){
  # Calculate NRMSE for the current column in the dataframe
  ARTH_NRMSEvalue <- nrmse.func(ARTH_SMBO2_DF[,i], pred = ARTH_SMBO2_DF$Total)
  # Get the name of the current dataframe column
  dataName <- unlist(strsplit(names(ARTH_SMBO2_DF)[[i]],'_'))
