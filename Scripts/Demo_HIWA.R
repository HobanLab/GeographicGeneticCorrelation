...
# HIWA_MultBuff_array <- readRDS(arrayDir)
# 
# # ---- CALCULATIONS ----
# # Build a data.frame from array values, to pass to linear models
# HIWA_MultBuff_DF <- resample.array2dataframe(HIWA_MultBuff_array)
...
# for(i in 3:ncol(HIWA_MultBuff_DF)){
#   # Calculate NRMSE for the current column in the data.frame
#   HIWA_NRMSEvalue <- nrmse.func(HIWA_MultBuff_DF[,i], pred = HIWA_MultBuff_DF$Total)
#   # Print result, for each explanatory variable in data.frame
#   print(paste0(names(HIWA_MultBuff_DF)[[i]], ': ', HIWA_NRMSEvalue))
# }
