...
# Geo Buff
matplot(AMTH_optCovMat[,c(1,2)], ylim=c(0,110), col=plotColors_Fade[c(1, 2)], pch=16, ylab='')
abline(h=95, col="black", lty=3)
abline(v=AMTH_Gen_MSSE, col="red") ; abline(v=AMTH_GeoBuff_MSSE, col="darkblue")
mtext(text=paste0('MSSE: ', AMTH_Gen_MSSE), side=1, line=-1, at=AMTH_Gen_MSSE+6, cex=0.8, col='red')
mtext(text=paste0(' MSSE: ', AMTH_GeoBuff_MSSE), line=-1.5, side=1, at=AMTH_GeoBuff_MSSE-6, cex=0.8, col='darkblue')
title('Amsonia tharpii: Coverages at Optimal Buffer Sizes', cex.sub=1.2, line = 2)
mtext(text='Geographic (Total Buffer): 30 km', side=3, at=15, cex=0.9)
# Legend
legend(x=100, y=95, xpd=TRUE, cex=1, pch=rep(19,3),
       col=c('red','darkblue','purple'),
       legend=c('Genetic', 'Geographic (Total Buffer)', 'Ecological'),
       y.intersp = 0.3, bty='n')
# Eco Buff
par(mar=c(3,4,2,1)+0.1)
matplot(AMTH_optCovMat[,c(1,3)], ylim=c(0,110), col=plotColors_Fade[c(1, 3)], pch=16, ylab='')
abline(h=95, col="black", lty=3)
abline(v=AMTH_Gen_MSSE, col="red") ; abline(v=AMTH_Eco_MSSE, col="purple")
mtext(text=paste0('MSSE: ', AMTH_Gen_MSSE), side=1, line=-1, at=AMTH_Gen_MSSE+6, cex=0.8, col='red')
mtext(text=paste0(' MSSE: ', AMTH_Eco_MSSE), line=-1.5, side=1, at=AMTH_Eco_MSSE-6, cex=0.8, col='purple')
mtext(text='Ecological: 35 km', side=3, at=15, cex=0.9)
