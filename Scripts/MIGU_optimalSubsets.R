...
# %%% RESAMPLING FOR TRULY RANDOMIZED GENETIC COVERAGES (10 REPLICATES) %%% ----
# Read in relevant functions
GeoGenCorr_wd <- '/home/akoontz/Documents/GeoGenCorr/Code/'
setwd(GeoGenCorr_wd)
source('Scripts/functions_GeoGenCoverage.R')
...
# %%% PLOTTING %%% ----
# %%% SEMI-RANDOMIZED VS. AND OPTIMIZED VALUES
...
MIGU_geoOptResults_SemiRand <- 
  cbind(c(7,11,49,59,63,67),MIGU_geoOptSubsets_Avs, MIGU_geoSemiRandSubsets_Avs)
colnames(MIGU_geoOptResults_SemiRand) <- c('SampleNumber','Optimized', 'Semi-Randomized')
# Build a plot from the matrix
matplot(MIGU_geoOptResults_SemiRand[,1],MIGU_geoOptResults_SemiRand[,2:3], pch=16, 
...
legend(x=40, y=80, inset = 0.05,
       legend = c('Optimized Sampling', 'Randomized Sampling'),
       col=c('red', 'black'), pch = c(20,20), cex=1.2, pt.cex = 2, bty='n',
       y.intersp = 0.8)
mtext(text='10 replicates', side=3, line=0.3, cex=1.3)

# %%% SEMI-RANDOMIZED, RANDOMIZED, AND OPTIMIZED VALUES
# Generate average genetic coverage values, across replicates, for randomized and optimized sampling
MIGU_geoOptSubsets_Avs <- apply(MIGU_geoOptSubsets, 1, mean)
MIGU_geoSemiRandSubsets_Avs <- apply(MIGU_geoSemiRandSubsets, 1, mean)
