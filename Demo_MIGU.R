...
# for Mimulus guttatus. Uses data files from Vallejo-Martin et al. 2021--a VCF, for genetic data, and a spreadsheet
# that was derived from supplemental data from the manuscript. This dataset is unique in that it's global, and coordinates
# are only provided at the population-level, not at the individual-level.
...
MIGU_vcf <- read.vcfR(file=paste0(MIGU_filePath, 'Genetic/mgut_all_20180305_gut_filter_75.i50.recode.pruned.plink_20180326.vcf'))
...
MIGU_genind <- vcfR2genind(MIGU_vcf, sep = "/", return.alleles = TRUE)
...
# and longitudes for each individual, individual names were added to the spreadsheet and matched manually to the lat/long values 
# for each population. 
MIGU_coordinates <- 
...
mtext(text='474 Individuals; 50 km buffer; 5 replicates', side=3, line=0.3, cex=1.3)
...
legend(x=205, y=60, inset = 0.05,
       legend = c('Genetic coverage (Total)', 'Geographic coverage (50 km buffer)', 'Ecological coverage (TNC ecoregions)'),
